//
//  ModelScopeProvider.swift
//  Icons Free Version - ModelScope Provider
//
...
class ModelScopeProvider: BaseAIProvider {
    ...
    private let baseURL = "https://api-inference.modelscope.cn/v1"
    ...
    override func generateImage(request: GenerationRequest) async throws -> GenerationResult {
        ...
        do {
            ...
            let response = try JSONDecoder().decode(ModelScopeResponse.self, from: data)
...
// MARK: - ModelScope Response Models

struct ModelScopeResponse: Codable {
    let created: Int
    let data: [ModelScopeImageData]
}

struct ModelScopeImageData: Codable {
    let b64_json: String
